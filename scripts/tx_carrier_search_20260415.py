#!/usr/bin/env python3
"""
BrokerOps AI – Texas carrier sourcing, 2026-04-15.
...
Usage:
    PYTHONPATH=. python scripts/tx_carrier_search_20260415.py [--dry-run]
...
_log_path = LOG_DIR / "tx_carrier_search_20260415.log"
...
log = logging.getLogger("tx_carrier_search")
...
def phase7_summary(stats: dict[str, dict], elapsed: float, dry_run: bool) -> None:
    log.info("=" * 60)
    log.info("Phase 7: Summary — TX carrier search 2026-04-15")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="TX carrier search 2026-04-15")
    parser.add_argument("--dry-run", action="store_true")
    args = parser.parse_args()
    t0 = time.time()
    log.info("Starting TX carrier search (dry_run=%s)", args.dry_run)
